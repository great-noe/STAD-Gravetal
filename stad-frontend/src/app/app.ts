...
// ── Validador Personalizado: Verifica que ETA > fechaSalida ──────────────────
// Función pura exportable fuera de la clase — patrón estándar Angular para
...
export class App implements OnInit, AfterViewInit {
  ...
  // ── Validación: Fecha mínima para los datetime-local (hoy, sin pasado) ───────────
  fechaMinimaActual: string = new Date().toISOString().slice(0, 16);

  // ── Catálogo de Embarcaciones de la Hidrovía ──────────────────────────────
  listaBarcazas: string[] = [
    'Naviera Chaco I',
    'Barcaza Río Paraguay',
    'Empuje Don Robert',
    'UABL-502',
    'Imperial S.A.',
    'Fluviomar Asunción',
    'Granelero Paraná III',
    'Hidrovía Express',
    'Companía Fluvial del Norte',
    'Delta Cargo II',
    'Barcazón San Lorenzo',
    'Remolcador Cono Sur'
  ];

  // ── Registros de Audiítía del Sistema (Mock) ────────────────────────────
  registrosAuditoria = [
    { fecha: '2026-03-30 08:14:22', usuario: 'noel.admin',     accion: 'Inicio de Sesión',           ip: '192.168.1.10' },
    ...
    { fecha: '2026-03-30 10:30:11', usuario: 'd.paredes',      accion: 'Exportación de Reporte PDF',   ip: '10.0.0.56'    },
    { fecha: '2026-03-30 11:00:00', usuario: 'r.acosta',       accion: 'Registro de Lote L-2026-004', ip: '192.168.1.24' },
    { fecha: '2026-03-30 11:47:19', usuario: 'noel.admin',     accion: 'Deshabilitación de Usuario p.estigarribia', ip: '192.168.1.10' },
    ...
    { fecha: '2026-03-30 14:10:05', usuario: 's.benitez',      accion: 'Inicio de Sesión',           ip: '10.0.0.55'    },
    { fecha: '2026-03-30 15:55:33', usuario: 'noel.admin',     accion: 'Generación de PDF de Audiítía', ip: '192.168.1.10' },
  ];
  ...
  // ── Ruta de la Hidrovía (compartida entre el mapa y el motor de interpolación) ─────
  private readonly rutaHidrovia: L.LatLngExpression[] = [
    ...
    [-27.3167, -58.5833], // Confluencia Paraguay-Paraná
  ...
  loteForm = new FormGroup(
    ...
    { validators: fechasViajeValidator }  // ← Validador cruzado a nivel de grupo
  );
  private map: any;
  ...
  async ngOnInit() {
    ...
    // ── Dashboard: Vista inicial según rol ────────────────────────────────
    if (this.isOperador) {
    ...
    // ── Perfil de usuario: cargar desde Keycloak ──────────────────────────
    try {
      ...
      if (this.isAdmin)         this.cargoUsuario = 'Gerente de Logística';
  ...
  // ── Dashboard: Cambiar la vista activa ───────────────────────────────────
  ...
  // ── Motor de Posicionamiento Dinámico: Dibuja barcos sobre la ruta según progreso temporal ──
  dibujarMarcadoresEnMapa() {
    ...
    for (const lote of this.lotes) {
      // Coordenadas de origen fijo: Puerto Quijarro (inicio de la Hidrovía)
      const origenLat = -18.9667;
      const origenLng = -57.7333;

      // Analizar fechas de planificación del lote
      ...
      if (tieneFechas) {
        ...
        if (ahora < fechaSalida) {
          // Aún no salió: aparece en el puerto de origen
          posicion = L.latLng(origenLat, origenLng);
          progresoPct = 0;
          estadoLabel = '⏳ En Puerto (Pendiente)';
        } else if (ahora >= fechaEntrega) {
          // Ya llegó: aparece en el destino registrado
          posicion = L.latLng(lote.latitud ?? origenLat, lote.longitud ?? origenLng);
          progresoPct = 100;
          estadoLabel = '✅ Entregado';
        } else {
          // En tránsito: interpolar sobre la ruta de la Hidrovía
        ...
        // Sin fechas: usar coordenadas estáticas del lote (comportamiento anterior)
        posicion = L.latLng(lote.latitud, lote.longitud);
        estadoLabel = '🚢 En Tránsito';
      } else {
        continue; // Sin datos geográficos: omitir
      }
  ...
  // ── Helper: Interpola un punto sobre una polilínea dada un progreso 0.0–1.0 ─────────
  // Usa distanciaTotal real (métrica) para un posicionamiento geográficamente preciso.
  private interpolarEnRuta(ruta: L.LatLngExpression[], progreso: number): L.LatLng {
    ...
    for (let i = 0; i < segmentos.length; i++) {
      if (acumulado + segmentos[i] >= distanciaObjetivo) {
        // Interpolación lineal dentro del segmento encontrado
  ...
  // ── Mapa: Volar a la ubicación de un lote específico ─────────────────────
  verEnMapa(lote: any) {
    ...
    setTimeout(() => {
      // 3. Corregir el tamaño del mapa por si estaba oculto
      ...
      L.popup()
        ...
        .setContent(
          ...
          '<span style="color:#28a745; font-weight:600;">🚢 Lote Seleccionado</span>'
        )
        .openOn(this.map);
    }, 200);
  }

  // ── Auditoría: Exportar PDF con jsPDF + autoTable ────────────────────────
  exportarAuditoriaPDF() {
    ...
    doc.text('GRAVETAL — Sistema STAD', 14, 13);

    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('Reporte de Auditoría del Sistema', 14, 21);
    ...
    autoTable(doc, {
      startY: 50,
      head: [['Fecha / Hora', 'Usuario', 'Acción', 'Dirección IP']],
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.text(`Página ${i} de ${totalPages}  |  GRAVETAL S.A. — Confidencial`, 14, 290);
    }
